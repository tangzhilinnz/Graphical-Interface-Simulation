...
bool Application::initApplication(HINSTANCE hInstance, const uint32_t& width, const uint32_t& height) {
	...
	*/
	...
	// Selects the mhBmp bitmap into the memory device context (mCanvasDC).
	// This means any drawing operation performed on mCanvasDC will now affect
	// the mhBmp bitmap.
...
BOOL Application::createWindow(HINSTANCE hInstance)
{
	...
	*/
	...
	ShowWindow(mHwnd, true); //显示窗体
	UpdateWindow(mHwnd); //刷新窗体
...
void Application::handleMessage(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_MOUSEMOVE: {
	    // If the cursor stops moving, and no further WM_MOUSEMOVE messages are
		// generated, the last set of coordinates received from GET_X_LPARAM
		// and GET_Y_LPARAM will be the final position of the cursor when it
		// stopped moving.
		// Windows sends a WM_MOUSEMOVE message every time the mouse is moved,
		// even slightly. If the cursor is held still, no WM_MOUSEMOVE messages
		// are sent, so the last received coordinates would be where the cursor
		// last moved to.
		if (mCamera) {
...
void Application::show() {
	// Double Buffering: To reduce flickering, BitBlt is often used to draw
	// content on an off-screen buffer before copying it to the display
	//
	// Common codes include:
	// SRCCOPY (direct copy),
	// SRCAND (AND the source and destination)
	// SRCPAINT (OR the source and destination).
