...
void Clipper::doClipSpace(
	const uint32_t& drawMode,
	const std::vector<VsOutput>& primitives,
	std::vector<VsOutput>& outputs) {
	//保证输出为空
	outputs.clear();

	if (drawMode == DRAW_TRIANGLES) {
		std::vector<VsOutput> primitive; // 存储了从primitives剥离出一个三角形的三个顶点
...
void Clipper::sutherlandHodgman(const uint32_t& drawMode, const std::vector<VsOutput>& primitive, std::vector<VsOutput>& outputs) {
	...
	static std::vector<math::vec4f> clipPlanes = {
		//judge w > 0
...
bool Clipper::inside(const math::vec4f& point, const math::vec4f& plane) {
	// 因此，点P在平面A上的充分必要条件就是平面A 向量与P的齐次坐标P'的内积等于0
