...
VsOutput LightShader::vertexShader(
	...
	math::vec4f normal_homogeneous = math::vec4f(normal, 0);
	normal_homogeneous = /*normal*/mModelMatrix * normal_homogeneous;
	output.mNormal = math::vec3f(normal_homogeneous);
...
void LightShader::fragmentShader(const VsOutput& input, FsOutput& output, const std::map<uint32_t, Texture*>& textures) {
	...
	diff = std::clamp(diff, 0.0f, 1.0f); // remove negative value
