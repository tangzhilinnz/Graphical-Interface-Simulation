...
math::vec4f Texture::getColor(float u, float v) {
	...
	if (mFilter == TEXTURE_FILTER_NEAREST) {
		...
		x = std::clamp(x, 0.f, static_cast<float>(mWidth - 1));
		y = std::clamp(y, 0.f, static_cast<float>(mHeight - 1));
		...
		left = std::clamp(left, 0, static_cast<int>(mWidth - 1));
		right = std::clamp(right, 0, static_cast<int>(mWidth - 1));
		bottom = std::clamp(bottom, 0, static_cast<int>(mHeight - 1));
		top = std::clamp(top, 0, static_cast<int>(mHeight - 1));
